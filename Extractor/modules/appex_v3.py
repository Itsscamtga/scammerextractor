...
async def appex_v3_txt(app, message, api, name):
    ...
    if "*" in raw_text:
        ...
        try:
            ...
            return await message.reply_text(
                "Please try again later. Maybe Password Wrong"
            )
    ...
    try:
        mc1 = requests.get(
            f"{api_base}/get/get_all_purchases?userid={hdr1['User-ID']}&item_type=10",
            headers=hdr1,
        ).json()
    except json.JSONDecodeError as e:
        print(f"JSON decode error: {str(e)}")
        return await message.reply_text(
            "Error decoding response from server. Please try again later."
        )
    except Exception as e:
        print(f"An error occurred: {str(e)}")
        return await message.reply_text(
    ...
    if "data" in mc1 and mc1["data"]:
        ...
        try:
            mc2 = requests.get(
                f"{api_base}/get/mycourseweb?userid={hdr1['User-ID']}", headers=hdr1
            ).json()
    ...
    editable2 = await app.send_message(
        message.chat.id, "**Now send the Course ID to Download**"
    )
    ...
    try:
        r = requests.get(
            f"{api_base}/get/course_by_id?id={raw_text2}", headers=hdr1
        ).json()
    except json.JSONDecodeError as e:
        print(f"JSON decode error: {str(e)}")
        return await message.reply_text(
        ...
        print(f"An error occurred: {str(e)}")
        return await message.reply_text(
            "An error occurred while fetching the course details. Please try again later."
        )
    ...
    for i in r.get("data", []):
        ...
        async with aiohttp.ClientSession() as session:
            with open(filename1, "w") as f:
                try:
                    r1 = await fetch(
                        session,
                        f"{api_base}/get/allsubjectfrmlivecourseclass?courseid={raw_text2}&start=-1",
                        hdr1,
                    )
                    tasks = []
                    for i in r1.get("data", []):
                        ...
                        r2 = await fetch(
                            session,
                            f"{api_base}/get/alltopicfrmlivecourseclass?courseid={raw_text2}&subjectid={si}&start=-1",
                            hdr1,
                        )
                        for i in r2.get("data", []):
                            ...
                            tasks.append(
                                handle_course(
                                    session, api_base, raw_text2, si, ti, hdr1, f
                                )
                            )
                    ...
                    return await message.reply_text(
                        "An error occurred while processing the course. Please try again later."
                    )
        ...
        try:
            ...
            await message.reply_text(
                "An error occurred while sending the document. Please try again later."
            )
        finally:
