...
async def rgvikram_down(
    app, message, hdr1, api, raw_text2, fuk, batch_name, name, prog
):
    vt = ""
    try:
        xx = fuk.split("&")
        for f in xx:
            res3 = requests.get(
                f"https://{api}/get/alltopicfrmlivecourseclass?courseid="
                + raw_text2
                + "&subjectid="
                + f
                + "&start=-1",
            ...
            try:
                ...
                for y in range(len(xv)):
                    t = xv[y]
                    res4 = requests.get(
                        f"https://{api}/get/livecourseclassbycoursesubtopconceptapiv3?courseid="
                        + raw_text2
                        + "&subjectid="
                        + f
                        + "&topicid="
...
async def rgvikram_txt(app, message, api, name):
    ...
    input2 = await app.ask(
        message.chat.id, text="**Now send the Batch ID to Download**"
    )
    ...
    scraper = cloudscraper.create_scraper()
    html = scraper.get(
        f"https://{api}/get/allsubjectfrmlivecourseclass?courseid={raw_text2}",
        headers=hdr1,
    ).content
